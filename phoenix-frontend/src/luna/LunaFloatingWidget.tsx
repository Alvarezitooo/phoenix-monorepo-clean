  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    if (luna.authenticatedUser) {
      return [
        {
          id: '1',
          text: `Salut ${luna.authenticatedUser.profile?.name || luna.authenticatedUser.email} ! 🌙 Je suis Luna, ton copilote IA. Tu as ${luna.lunaEnergy}⚡ d'énergie. Comment puis-je t'aider aujourd'hui ?`,
          sender: 'luna',
          timestamp: new Date()
        }
      ];
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (luna.authenticatedUser) {
        ...
        responseText = `Je comprends ta question "${userMessage.text}". Je suis connectée et je vois que tu es ${luna.authenticatedUser.profile?.name || luna.authenticatedUser.email} avec ${luna.lunaEnergy}⚡ d'énergie ! 🌟\n\nBientôt je pourrai accéder à ton historique et tes préférences. En attendant, que puis-je faire pour t'aider ?`;
      } else {
        // User not authenticated
        responseText = `Je comprends ta question "${userMessage.text}". Pour mieux t'aider, connecte-toi d'abord ! Une fois authentifié, j'aurai accès à ton profil et je pourrai te donner des conseils personnalisés ! 🚀`;
      }
